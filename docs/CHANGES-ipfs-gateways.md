...
## Update, 2026-09-23: hypha cannot retrieve our content

Some .kas sites appeared to fail on the first mirror and to load only after
clicking "Try another mirror" by hand. The cause was not slowness.

`ipfs.hypha.coop` sends **zero bytes** for every one of our CIDs, at an 8
second timeout and at 15 seconds alike, and over four attempts spread across
two and a half minutes. Its HTTP layer is healthy: `/` answers 404 in about
0.9s, TLS completes in 0.57s, and it serves an unrelated well-provided CID in
under a second. It specifically cannot *retrieve* our content.

The provider records explain why:

| CID | Providers announced |
| --- | --- |
| `examplesite.kas` (`bafkreia2mz...`) | 2, both Pinata: `/dns4/bitswap-v3.pinata.cloud/tcp/443/wss` and `/dns4/gateway-v3.pinata.cloud/tcp/443/https` |
| `rescu.kas` (`bafybeibg6g...`) | 4: the two Pinata entries, plus `91.233.10.102` (not serving) and `140.235.158.66`, which is hypha's own stale self-announcement |
| ipfs.tech site (control) | 19, many plain TCP and QUIC peers |

Our content is announced almost only by Pinata, over WebSocket-Secure and
HTTPS bitswap transports that a conventional Kubo gateway does not dial.
Filebase can fetch it; hypha cannot, so it hangs past its own internal
timeout. Three of the four .kas names tested point at raw single-file CIDs
(`bafkrei...`), though the codec is not the problem: the dag-pb control fails
on hypha too.

This also corrects the 2026-09-21 measurement in this document. hypha looked
like the one passing gateway partly because those very probes warmed its
cache, and it is still listed as a provider for `rescu.kas` from that episode
while no longer being able to serve it. The "23.7s cold, 2.5s warm" figure was
hypha caching what it had just been asked for, not evidence that it can
retrieve our content.

### Measured, live, per site

| Site | hypha subdomain | filebase path (warm) |
| --- | --- | --- |
| `rescu.kas` (dag-pb, control) | no bytes, 60s timeout | 0.67s TTFB, 200 |
| `examplesite.kas` (raw) | no bytes, 60s timeout | 0.72s TTFB, 200 |
| `webclient.kas` (raw) | no bytes, 60s timeout | 4.38s TTFB, 200 |
| `landing.kas` (raw) | no bytes, 60s timeout | 0.74s TTFB, 200 |

### The change: race the probes

The web client now probes every gateway concurrently instead of in order, and
picks a winner by preference: a script-capable gateway that passes wins
outright; a script-restricted one that passes is held for up to
`HEAD_START_MS` (3s) in case a script-capable one is merely slower; an unknown
gateway is a last resort; otherwise the existing failure panel shows. Whether
a gateway is script-restricted is now a per-gateway flag rather than filebase
being named in the logic. "Try another mirror" became a true manual override:
it switches outright without re-probing, since the race has already decided.

The per-probe timeout went from 8s to 15s. That is safe now only because the
probes race: a longer timeout no longer delays anything, since a fast gateway
wins immediately. Raising it further would not recover hypha, because the
failure is retrieval, not latency.

End-to-end pick time against the live gateways:

| Site | serial at 8s (before) | raced, 3s head start (now) | raced, no head start |
| --- | --- | --- | --- |
| `rescu.kas` | 11.1s | 3.9s | 0.43s |
| `examplesite.kas` | 8.9s | 3.7s | 0.27s |
...
site is served path style from a script-restricted mirror. The script warning
shows constantly, and any site relying on inline scripts will have them
blocked. The head start costs about 3.4s on every load and currently buys
nothing, because the script-capable gateway it waits for never answers.

The real fix is at the publishing layer, not in the client: the content needs
providers that ordinary gateways can dial, over plain TCP or QUIC, rather than
being announced only over Pinata's wss and https bitswap transports. Until
then, either accept filebase as the effective primary and set `HEAD_START_MS`
to 0 to recover those 3.4s, or add a gateway that can retrieve
Pinata-announced content.
